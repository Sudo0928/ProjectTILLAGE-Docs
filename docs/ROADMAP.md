...
- **F001 글 목록 조회**: Notion DB의 발행 상태 글을 카드 그리드로 노출
- **F002 글 상세 렌더링**: Notion 페이지 블록을 HTML로 변환해 본문 표시
- **F003 카테고리 필터링**: Notion select 옵션을 카테고리 메뉴로 동기화
...
- **Task 003: 도메인 타입 및 Zod 스키마 정의**
  - `src/types/post.ts` 에 `Post`, `PostContent`, `Category`, `NotionBlock` 타입 정의 (PRD 데이터 모델 기반)
  - `src/lib/notion/schemas.ts` 에 Notion 응답 정규화용 Zod 스키마 정의 (title, select, multi_select, date 등)
  - 검색 쿼리 파라미터(`SearchParams`) 타입과 Zod 검증 스키마 정의
  - 카드 컴포넌트가 사용할 `PostCardProps` 인터페이스 사전 정의
  - 산출물: 데이터 레이어와 UI가 공유할 단일 타입 소스
...
- **Task 005: 글 목록/상세 조회 함수 구현 (F001/F002)**
  - `getPublishedPosts()` 구현: `Status=발행됨` 필터 + 발행일 내림차순 정렬
  - `getPostBySlug(slug)` 구현: 페이지 메타와 `blocks.children.list` 호출 후 정규화
  - `getCategories()` 구현: select 옵션 + 카테고리별 글 수 집계
...
- **Task 006: Notion 블록 → HTML 변환 유틸 구현 (F002)**
  - 지원 블록: `paragraph`, `heading_1~3`, `bulleted_list_item`, `numbered_list_item`, `image`, `code`, `quote`, `divider`
  - 텍스트 어노테이션(볼드/이탤릭/코드/링크) 처리
  - 이미지 블록 → `next/image` 호환 컴포넌트 출력 (커버 이미지 포함)
  - 미지원 블록은 콘솔 경고 + 안전한 fallback 렌더링
  - 산출물: `src/lib/notion/render-blocks.tsx` (서버 컴포넌트로 사용 가능)
...
- **Task 008: 글 카드 및 상태 컴포넌트 (F010/F012/F013)**
  - `PostCard`: 제목, 카테고리 배지, 태그 칩, 발행일, 본문 요약, 호버 강조
...
- **Task 010: 홈 페이지 구현 (F001/F010/F012/F013)**
  - 서버 컴포넌트에서 `getPublishedPosts()` 호출 후 `PostCardGrid` 렌더링
  - 첫 화면 헤드라인/서브카피 정리
  - 빈/에러/로딩 상태 컴포넌트 연결
  - 카드 클릭 시 `/posts/[slug]` 라우팅 동작 확인
  - **Playwright MCP 테스트**: 홈 → 카드 클릭 → 상세 동선 무오류 검증
...
- **Task 013: 글 상세 페이지 구현 (F002/F010/F012/F013/F014)**
  - 슬러그로 메타+본문 조회 후 헤더(제목/발행일/카테고리/태그) + 본문(`prose` + 변환 유틸) 렌더링
  - 본문 가독성: 모바일 16px 이상 / 키보드 포커스 스타일 / 명도 대비 확보
  - 본문 로딩 실패 시 `ErrorState` + 홈 이동 버튼
  - `generateMetadata`로 글 제목/요약 기반 SEO 태그 생성 (PRD F014)
  - OpenGraph/Twitter 카드용 절대 URL은 `NEXT_PUBLIC_SITE_URL` 사용
  - **Playwright MCP 테스트**: 카드 클릭 → 본문 정상 표시, 잘못된 슬러그 시 에러 UI 노출, OG 메타 태그 확인
